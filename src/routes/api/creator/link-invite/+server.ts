...
import { creatorInstagramUsername, requireAuthenticatedCreator } from '$lib/server/creatorAuth';
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  const creator = await requireAuthenticatedCreator(sb, request);
  const creatorUsername = creatorInstagramUsername(creator.profileData);
  if (!creatorUsername) throw error(403, 'creator_instagram_not_linked');

  const { data: rosterRow, error: rosterLookupError } = await sb
  ...
  if (rosterLookupError) {
    console.error('[creator/link-invite] roster lookup failed:', rosterLookupError.message);
    throw error(500, 'roster_lookup_failed');
  }
  if (!rosterRow) throw error(404, 'invite_not_found');
  if (String(rosterRow.ig_username ?? '').toLowerCase() !== creatorUsername) {
    throw error(403, 'invite_creator_mismatch');
  }

  const { data: linkedRow, error: linkError } = await sb
    .from('brand_creator_roster')
    .update({
      status: 'on_platform',
  ...
  if (linkError) {
    console.error('[creator/link-invite] roster link failed:', linkError.message);
    throw error(500, 'invite_link_failed');
  }
  if (!linkedRow) throw error(409, 'invite_already_claimed');
