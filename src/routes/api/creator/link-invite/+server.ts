...
export const POST: RequestHandler = async ({ request }) => {
  ...
  const creatorIgUsername = profileInstagramUsername(creator.profileData);
  if (!creatorIgUsername) throw error(403, 'creator_instagram_not_connected');

  // 1. Claim exactly the pending roster invite for this authenticated creator.
  ...
  const rosterUsername =
    typeof rosterRow.ig_username === 'string'
      ? rosterRow.ig_username.trim().replace(/^@/, '').toLowerCase()
      : '';
  if (!rosterUsername || rosterUsername !== creatorIgUsername) {
  ...
  if (claimErr) {
    console.error('[creator/link-invite] roster claim failed:', claimErr.message);
    throw error(500, 'invite_claim_failed');
  }
  if (!claimed) throw error(409, 'invite_already_claimed');
