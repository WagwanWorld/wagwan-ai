...
import { partitionRowsByRosterMembership } from '$lib/server/marketplace/bulkRoster';
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  const { toProcess, alreadyInRoster } = await partitionRowsByRosterMembership(sb, brandId, valid);
      ...
      for (let batchStart = 0; batchStart < total; batchStart += BATCH_SIZE) {
        ...
        const results = await Promise.allSettled(
          batch.map(async (row) => {
            ...
            if (updateError) {
              throw new Error('roster_sheet_merge_failed');
            }
