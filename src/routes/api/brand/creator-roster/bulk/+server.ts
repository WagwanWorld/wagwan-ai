...
} from '$lib/server/marketplace/bulkRosterUpload';
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  if (valid.length > 0) {
    ...
    try {
      existingSet = await loadExistingBrandRosterHandles(sb, brandId, handles);
    } catch (e) {
      console.error('[creator-roster bulk] existing lookup failed:', e);
      throw error(500, 'Could not check existing roster entries');
    }

    const partitioned = partitionRowsByExistingRoster(valid, existingSet);
    alreadyInRoster = partitioned.alreadyInRoster;
    toProcess.push(...partitioned.toProcess);
  }
