...
import { listExistingRosterHandlesForBrand } from '$lib/server/marketplace/bulkRoster';
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  if (valid.length > 0) {
    ...
    const existingSet = await listExistingRosterHandlesForBrand(sb, brandId, handles);
