<script lang="ts">
  ...
  async function finish() {
    ...
    // Link wagwan user if authenticated, then claim any roster invite for this creator.
    ...
    if (wToken && wSub) {
      ...
      wagwanLinked = Boolean(linkRes?.ok);
    }

    const invBrand = localStorage.getItem('wagwan_invite_brand');
    const invRoster = localStorage.getItem('wagwan_invite_id');
    if (wToken && wagwanLinked && invBrand && invRoster) {
      const claimRes = await fetch('/api/creator/link-invite', {
        method: 'POST',
