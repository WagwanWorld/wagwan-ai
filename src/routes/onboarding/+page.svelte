<script lang="ts">
  ...
  function finish() {
    ...
    try {
      ...
      profileSaved = saveRes.ok && saveData.ok !== false;
    } catch {
      profileSaved = false;
    }

    // Link wagwan user if authenticated; invite linkback relies on this server-side binding.
    if (wToken && wSub && profileSaved) {
      try {
        ...
        wagwanLinked = linkRes.ok && linkData.ok === true;
      } catch {
        wagwanLinked = false;
      }
    }
    ...
    if (invBrand && invRoster && wToken && wagwanLinked) {
      try {
        const inviteRes = await fetch('/api/creator/link-invite', {
          ...
          body: JSON.stringify({ brandId: invBrand, rosterId: invRoster }),
        });
        if (inviteRes.ok) {
        ...
        // Silent fail — creator still onboards fine, and invite state remains for retry.
      }
    }
