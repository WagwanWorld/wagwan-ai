  ...
  async function finish() {
    ...
    if (wToken && wSub) {
      try {
        ...
        wagwanLinked = profileSaved && linkRes.ok && linkData.ok === true;
      } catch {}
    }

    // Link invite back to roster + create creator signal after the profile is auth-bound.
    const invBrand = localStorage.getItem('wagwan_invite_brand');
    const invRoster = localStorage.getItem('wagwan_invite_id');
    if (invBrand && wToken && wagwanLinked) {
      try {
        ...
        if (inviteRes.ok && inviteData.ok === true) {
        ...
        // Keep invite keys so a future retry can still recover the link.
      }
    }
