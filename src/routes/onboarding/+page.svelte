<script lang="ts">
  ...
  async function finish() {
    ...
    try {
      ...
      // Keep onboarding usable if persistence is temporarily unavailable.
    }
    ...
    if (wToken && wSub) {
      try {
        ...
        // The invite claim below will no-op without a linked creator.
      }
    }

    // Link invite back to roster + create creator signal after the profile is saved and
    // linked, so the server can verify the bearer token belongs to this creator.
    const invBrand = localStorage.getItem('wagwan_invite_brand');
    const invRoster = localStorage.getItem('wagwan_invite_id');
    if (invBrand && invRoster && wToken) {
      try {
