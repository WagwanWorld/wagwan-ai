<script lang="ts">
  ...
  async function finish() {
    ...
    try {
      ...
      // Link Wagwan before claiming an invite so the server can verify the bearer token.
      if (wToken) {
      ...
      // Creator can still finish onboarding; the invite can be retried while localStorage keeps it.
    }
