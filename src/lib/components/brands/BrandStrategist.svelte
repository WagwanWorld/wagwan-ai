<script lang="ts">
  ...
  $: recentPosts = (intelligence?.recentPosts || []) as Array<{ id: string; thumbnail: string; type: string; likes: number; comments: number; permalink: string }>;
  ...
  $: profile = snapshot ? {
    ...
    profilePicture: snapshot.intelligence?.identity?.profilePicture || brandProfile.ig_profile_picture,
    ...
    try {
      ...
      if (!res.ok) {
        ...
        try {
          ...
          if (errData.needsReauth) { needsReauth = true; refreshError = errData.error; refreshing = false; return false; }
  ...
  async function handleRefresh() {
    ...
    if (!await runPhase('1', 'Fetching Instagram data...')) { refreshing = false; return; }
    await reloadData();

    // Phase 2a: Identity pipeline (~8s)
    if (!await runPhase('2a', 'Extracting brand identity...')) { refreshing = false; return; }
    await reloadData();

    // Phase 2b: Audience + content analysis (~8s)
    if (!await runPhase('2b', 'Analysing audience & content...')) { refreshing = false; return; }
    await reloadData();

    // Phase 2c: Strategy + brief (~8s)
    if (!await runPhase('2c', 'Generating strategy & brief...')) { refreshing = false; return; }
    await reloadData();

    // Phase 2d: Content proposals (~8s)
    if (!await runPhase('2d', 'Generating content ideas...')) { refreshing = false; return; }
    await reloadData();

    // Phase 2e: Creator matches (~8s)
    if (!await runPhase('2e', 'Finding creator matches...')) { /* non-critical, continue */ }
    await reloadData();
...
<div class="strategist">
  ...
  >
    ...
    <div class="prof-stats">
      <div class="pst">
        <Metric value={profile.followersCount.toLocaleString()} type="growth" size="lg" showLabel={false} />
        <div class="pst-l">Followers</div>
      </div>
      <div class="pst">
        <Metric value={profile.mediaCount.toLocaleString()} type="cadence" size="lg" showLabel={false} />
        <div class="pst-l">Posts</div>
      </div>
      <div class="pst">
        <Metric value="{snapshot?.engagement_rate || '0'}" suffix="%" type="engagement" size="lg" showLabel={false} />
  ...
  {#if refreshError}
    <div class="strat-banner" class:strat-banner--reauth={needsReauth}>
      ...
      {#if needsReauth}
        <a href="/brands/login?next=/brands/portal" class="banner-action">Re-authenticate</a>
      {:else}
  ...
  {#if dataError}
    <div class="strat-banner">
      ...
      <button class="banner-dismiss" on:click={() => { dataError = ''; reloadData(); }}>Retry</button>
    </div>
  {/if}
  ...
  <div class="grid">
    ...
    <GlassCard span={12} color="blue" expandable expandLabel="Read full brief" glowSize={115} delay={0.36}>
      {#if brief}
      ...
      {:else}
        <div class="section-empty">
          ...
          <button class="section-retry" on:click|stopPropagation={() => runPhase('2c', 'Generating brief...')} disabled={refreshing}>Generate Brief</button>
        </div>
      {/if}
    </GlassCard>
    ...
    <GlassCard span={3} color="red" glowSize={85}>
      ...
      <Metric value={snapshot?.reach_7d?.toLocaleString() || '—'} type="reach" size="xl" showLabel={false} />
      <div class="cd">Weekly reach</div>
    </GlassCard>

    <GlassCard span={3} color="green" glowSize={70} glowY="bottom">
      <div class="cl">Avg. Saves</div>
      <Metric value={snapshot?.avg_saves?.toLocaleString() || '—'} type="engagement" size="xl" showLabel={false} />
      <div class="cd">Per post</div>
    </GlassCard>

    <GlassCard span={3} color="indigo" glowSize={65} glowX="left">
      <div class="cl">Shares</div>
      <Metric value={snapshot?.avg_shares?.toLocaleString() || '—'} type="shares" size="xl" showLabel={false} />
    ...
    <GlassCard span={3} color="amber" glowSize={70} glowX="left" glowY="bottom">
      ...
      <div class="cd">{snapshot?.posts_per_week && snapshot.posts_per_week < 2 ? 'Increase recommended' : 'Current pace'}</div>
    </GlassCard>
    ...
    <GlassCard span={8} color="red" expandable expandLabel="Expand full strategy" glowSize={170} glowY="bottom" delay={0.42}>
      {#if strategicPositioning?.brandDirection || (intelligence?.identity)?.rawSummary}
        <StratBrandDirection
          brandDirection={strategicPositioning?.brandDirection && !strategicPositioning.brandDirection.includes('Not enough data') ? strategicPositioning.brandDirection : (intelligence?.identity)?.rawSummary || ''}
          audienceSummary={audiencePortrait?.narrative && !audiencePortrait.narrative.includes('not yet available') ? audiencePortrait.narrative : ''}
      ...
      {:else}
        <div class="section-empty">
          ...
          <button class="section-retry" on:click|stopPropagation={() => runPhase('2c', 'Generating strategy...')} disabled={refreshing}>Analyse Strategy</button>
        </div>
      {/if}
    </GlassCard>

    <!-- Posting Heatmap (span 4) -->
    <GlassCard span={4} color="rose" expandable expandLabel="Full heatmap" glowSize={85} glowX="left" delay={0.42}>
      ...
      {:else}
        <div class="section-empty">
          ...
          <button class="section-retry" on:click|stopPropagation={() => runPhase('1', 'Fetching data...')} disabled={refreshing}>Fetch Data</button>
        </div>
      {/if}
    </GlassCard>

    <!-- Audience DNA (span 6) -->
    <GlassCard span={6} color="teal" expandable expandLabel="Full demographics" glowSize={105} glowX="left" glowY="bottom" delay={0.48}>
      ...
      {:else}
        <div class="section-empty">
          ...
          <p class="section-empty-text">Not analysed yet.</p>
          <button class="section-retry" on:click|stopPropagation={() => runPhase('2b', 'Analysing audience...')} disabled={refreshing}>Analyse Audience</button>
        </div>
      {/if}
    </GlassCard>

    <!-- Engagement Breakdown (span 6) -->
    <GlassCard span={6} color="indigo" expandable expandLabel="Full engagement breakdown" glowSize={140} delay={0.48}>
      <StratEngagementBreakdown
        ...
        growthTrend={0}
        contentTypes={Object.entries(contentPerformance?.formatBreakdown || {}).map(([type, v]) => ({
          type,
          count: (v)?.count || 0,
      ...
      />
      {#if !snapshot}
        <button class="section-retry" on:click|stopPropagation={() => runPhase('1', 'Fetching metrics...')} disabled={refreshing}>Fetch Metrics</button>
      {/if}
    </GlassCard>
    ...
    <GlassCard span={6} color="purple" expandable expandLabel="See all ideas" glowSize={95} delay={0.54}>
      ...
      {:else}
        <div class="section-empty">
          ...
          <button class="section-retry" on:click|stopPropagation={() => runPhase('2d', 'Generating ideas...')} disabled={refreshing}>Generate Ideas</button>
        </div>
      {/if}
    </GlassCard>

    <!-- Creator Matches (span 6) -->
    <GlassCard span={6} color="emerald" expandable expandLabel="Browse all matches" glowSize={85} glowY="bottom" delay={0.54}>
      <div class="section-empty">
        <span class="section-empty-label">Creator Matches</span>
        {#if proposals.filter(p => p.type === 'creator_match').length > 0}
          <p class="section-empty-text">{proposals.filter(p => p.type === 'creator_match').length} matches found</p>
        {:else}
          <p class="section-empty-text">No matches yet.</p>
          <button class="section-retry" on:click|stopPropagation={() => runPhase('2e', 'Finding creators...')} disabled={refreshing}>Find Creators</button>
        {/if}
      </div>
    </GlassCard>

    <!-- Competitor Watch (span 12) — bottom utility -->
    <GlassCard span={12} color="gold" expandable expandLabel="Manage competitors" glowSize={125} delay={0.6}>
...
<style>
  ...
  .prof-accent-bg {
    ...
    background: linear-gradient(140deg, rgba(255,55,55,0.02), rgba(255,130,65,0.012), transparent 55%);
    pointer-events: none;
  }
  ...
  .prof-av-img {
    ...
    border-radius: 18px;
    object-fit: cover;
    box-shadow: 0 6px 28px rgba(255,70,70,0.12), 0 1px 2px rgba(0,0,0,0.2);
  }
  .prof-av-fallback {
    width: 68px; height: 68px;
    border-radius: 18px;
  ...
  .b {
    ...
    gap: 6px;
    box-shadow: var(--g-flat-shadow);
    transition: color 0.6s, background 0.6s, border-color 0.6s, box-shadow 0.6s;
  }
  .b:hover {
  ...
  .th-type { font-size: 9px; color: var(--g-text-3); text-transform: uppercase; letter-spacing: 0.08em; }
  ...
  @media (max-width: 900px) {
    ...
    .sec-label { grid-column: span 4; }
    .th { width: 88px; height: 88px; border-radius: 14px; }
  }
  @media (max-width: 600px) {
    .grid { grid-template-columns: repeat(2, 1fr); }
    .sec-label { grid-column: span 2; }
    .prof-name { font-size: 21px; }
    .prof-av-img, .prof-av-fallback { width: 52px; height: 52px; font-size: 18px; border-radius: 14px; }
    .prof-av-wrap { width: 52px; height: 52px; }
    .pst-v { font-size: 21px; }
    .cn { font-size: 34px; }
    .th { width: 76px; height: 76px; border-radius: 12px; }
  }
</style>
