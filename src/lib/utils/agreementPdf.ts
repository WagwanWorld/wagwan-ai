...
export function generateAgreementPdf(info: SigningInfo) {
  ...
  body(
    `This Service Agreement (hereinafter referred to as the "Agreement") is made and entered into on ${info.date}, by and between:`,
  );
  y += 2;
  ...
  body(
    `This Agreement shall remain in full force and effect for a period of three (3) years from the Effective Date, unless terminated earlier in accordance with the terms herein. Upon expiry, the Agreement may be renewed by mutual written consent of both Parties.`,
  );
  ...
  body(
    `5.5. In the event of payment default by ${cn}, the Service Provider shall provide a written notice granting a seven (7) day grace period.`,
  );
  body(
    `5.6. If ${cn} fails to make payment within the grace period, the Service Provider shall have the right to suspend access to the Platform.`,
  );
  body('5.7. During the grace period, services shall continue uninterrupted.');
  body(
    '5.8. Upon receipt of all outstanding payments, the Service Provider shall promptly reinstate access within two (2) business days.',
  );
  body(
    "5.9. Reinstatement shall not waive the Service Provider's right to claim interest or penalties from prior non-payment.",
  );
  ...
  body(
    '7.1. Term: This Agreement shall commence on the Effective Date and remain in effect for 12 months, unless terminated earlier.',
  );
  body(
  ...
  numberedItem(
    'a.',
    `${cn} shall immediately cease all use of the Platform and clear all outstanding financial dues.`,
  );
  numberedItem(
    'b.',
  ...
  body(
    `11.1. ${cn} agrees to indemnify, defend, and hold harmless the Service Provider from and against any and all claims arising out of:`,
  );
  numberedItem('a.', `Any breach of this Agreement by ${cn}.`);
  numberedItem('b.', `Any negligence by ${cn} in connection with Platform use.`);
  numberedItem('c.', 'Any claims from Customers, including refunds or event cancellations.');
  numberedItem('d.', `${cn}'s failure to comply with applicable tax or regulatory obligations.`);
  body(
    "e) This indemnity shall not extend to liability arising from the Service Provider's own acts or compliance failures.",
  );
  body('f) Each Party shall remain individually responsible for its respective regulatory duties.');

  // 12. Limitation
  heading('12. LIMITATION OF LIABILITY');
  body(
    '12.1. Total aggregate liability shall not exceed the total revenue generated by the Service Provider from the Platform in the preceding three (3) calendar months.',
  );
  body(
    `12.2. The liability cap is based on actual commissions and subscription revenues received from ${cn}'s use of the Platform.`,
  );
  body(
    '12.3. The Service Provider shall not be liable for any indirect, consequential, incidental, punitive, or special damages.',
  );
  body('12.4. This limitation applies to all claims under this Agreement.');
  body(
    `12.5. In the event of gross negligence or fraud, ${cn} shall provide written notice within two (2) months. The Service Provider shall be granted a one (1) month rectification period.`,
  );
  ...
  body('13.2. The affected Party shall promptly notify the other in writing.');
  body(
    '13.3. If the Force Majeure Event continues for sixty (60) consecutive days, either Party may terminate upon written notice.',
  );
  ...
  body(
    '15.1. Disputes shall be resolved by arbitration under the Arbitration and Conciliation Act, 1996.',
  );
  body(
    '15.2. The arbitration shall be conducted by a sole arbitrator mutually appointed within fifteen (15) days.',
  );
  body(
    '15.3. If the Parties cannot agree on an arbitrator, appointment shall follow Section 11 of the Act.',
  );
  body(
    '15.4. Seat and venue: Bengaluru, Karnataka. Proceedings in English. The arbitral award shall be final and binding.',
  );
